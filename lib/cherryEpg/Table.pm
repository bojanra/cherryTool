...
sub _SDT {
  ...
  my @requiredKeyInService = qw(service_id eit_schedule_flag eit_present_following_flag running_status free_ca_mode descriptors);
  my $services             = '';
...
sub _PMT {
  ...
  foreach my $stream ( $table->{elementary_streams}->@* ) {
    ...
    $bin .=
        pack( "Cnna*", $stream->{stream_type}, 0xe000 | $stream->{elementary_pid}, 0xf000 | length($descriptor), $descriptor );
