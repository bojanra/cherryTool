...
sub BUILD {
    ...
    $self->{source}     = dir( $self->cherry->config->{cherryepg}{ingest}, $self->{channel_id} );
...
sub ingest {
    ...
    while ( $i < scalar(@$eventList) ) {
        ...
        if ( exists $$eventList[$i]->{stop} ) {
            ...
            if ( ( $$eventList[$i]->{stop} - $$eventList[$i]->{start} ) > 24 * 60 * 60 ) {
                $$eventList[$i]->{stop} = $$eventList[$i]->{start} + 24 * 60 * 60;
            }
        } ## end else [ if ( exists $$eventList...)]
        $i += 1;
    ...
    foreach my $event (@$eventList) {
        ...
        $short_descriptor->{language_code} = $self->{language};
    ...
    if ( $logger->is_trace() ) {
        ...
        my $output = YAML::XS::Dump($result);
        utf8::decode($output);
        say $output;
    } ## end if ( $logger->is_trace...)
...
sub parseFile {
    ...
    my $parsedData = try {
        ...
        $logger->error( "parse [$filename] - $reason", $self->{channel_id}, );
        return;
    };
                ...
                if ( !-e $ctrlFile or compare( $md5File, $ctrlFile ) != 0 ) {
                    ...
                    if ( -e $dataFile ) {
                        ...
                        if ($parsedData) {
                            ...
                            # ingest / don not ingest if dryrun
                            if ( !$self->dryrun ) {
