...
Delete all entries in section and version table
...
sub addChannel {
    ...
    my ($count) = $dbh->selectrow_array("SELECT COUNT(*) FROM channel WHERE channel_id=$channel_id");
    my $insertOrUpdate;
...
sub updateEit {
    ...
    while ( $rule = $sel->fetchrow_hashref ) {
        ...
        $ret = $self->updateEitPresent( $rule, $forced, $currentTime );
        if ( !defined $ret ) {
            return;
        }
        $updated |= $ret;
        ...
        if ( exists $rule->{maxsegments} and $rule->{maxsegments} > 0 ) {
            $ret = $self->updateEitSchedule( $rule, $forced, $currentTime );
            if ( !defined $ret ) {
                return;
            }
            $updated |= $ret;
...
sub updateEitPresent {
    my ( $self, $rule, $forced, $currentTime ) = @_;
    my $dbh = $self->dbh;
    return unless $dbh;
    ...
    # return unless $insert;   # TODO what if failed
...
=head3 updateEitSchedule( $rule, $forced)
...
sub updateEitSchedule {
    ...
    while ( $subtable_count <= $num_subtable ) {
        ...
        # Remove all section of this table
        # TODO return unless $dbh->do( "DELETE FROM section WHERE
        $dbh->do(
...
sub getSectionFrequency {
    ...
    # EIT p/f other               - every <10s = 8s
    return ceil( $timeFrame / 7 ) if $table_id == 0x4f;

    return ceil( $timeFrame / 10 ) if ( $table_id == 0x50 ) and ( $section_number < ( 1 * 24 / 3 ) );    # days * 24 / 3
        # EIT p/f actual              - every <2s
    return ceil( $timeFrame / 30 );

    # return ceil($timeFrame / 10) if ( $table_id == 0x50) and ( $section_number < (1 * 24 / 3 )); # days * 24 / 3
    # 1 seg = 3 ure > 32 seg = 96 ur ( 4dni) = 1 tabela
    # 2 tabeli = 8dni
    # schedule repetition 10s za other in actual
    return ceil( $timeFrame / 10 ) if $table_id == 0x4f;
    return ceil( $timeFrame / 10 ) if ( $table_id & 0x0f ) <= 1;    # days * 24 / 3
    return ceil( $timeFrame / 30 );
...
Return list of log entries filtered by params
