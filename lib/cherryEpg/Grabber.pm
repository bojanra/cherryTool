...
sub BUILD {
    ...
    $self->{source}      = $self->cherry->config->{cherryepg}{stock};
    $self->{destination} = dir( $self->cherry->config->{cherryepg}{ingest}, $self->{channel_id} );
} ## end sub BUILD
...
sub grab {
    ...
    if ( $self->{url} !~ m|://| ) {
        ...
        $logger->trace( "use rsync method", $self->{channel_id} );
    }
...
sub _rsync {
    ...
    my $source = file( $self->{source}, $self->{uri}->opaque );
    ...
    if ( !$rsync->exec() or $rsync->status != 0 ) {
...
sub _curl {
    ...
    for ( my $i = 0 ; $i < $days && $i < 10 ; $i++ ) {
        ...
        my $filename;
        if ( $response_header =~ /^Content-Disposition: .*filename="(.+)"/m ) {
            $filename = $1;
        } else {
