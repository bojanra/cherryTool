...
sub getShortEventDescriptorBin {
    ...
    if ( $raw_event_name ne "" ) {
        ...
        $event_name = $self->{codepage_prefix} . substr( $encoded, 0, 255 - 5 - $self->{codepage_prefix_length} );
    }
    ...
    if ( $raw_text ne "" ) {
        ...
        $text =
            $self->{codepage_prefix} . substr( $encoded, 0, 255 - 5 - $event_name_length - $self->{codepage_prefix_length} );
    }
...
sub getExtendedEventDescriptorBin {
    ...
    while ( $descriptor_number <= $last_descriptor_number ) {
        ...
        if ( $descriptor_number == 0 ) {
            ...
            # clear items after first iteration
            $items_length = 0;
            $items        = '';
